...
void API::create_index(string index_name, string table_name, string attribute_name)
{
	...
	if(record_manager->indexCreate(index_name)){ // 建立文件
		...
		// int bn = ;
		record_manager->insertNewIndex(table_name, recordSize, bn, attribute_vector, i);
				...
				for(j = 0; j < attribute_vector.size(); j++){
					if(attribute_vector[j].name == condition_vector[i].Attribute_Name){
						if(condition_vector[i].operate == OPERATOR_EQUAL && attribute_vector[j].index != ""){
							offset = index_manager->searchIndex(index_file_name_get(attribute_vector[j].index), condition_vector[i].value, attribute_vector[j].type);
						}
						break;
					}
				}
...
void API::record_insert(string table_name, vector<string> &value_vector)
{
	...
	catalog_manager->getAttributes(attribute_vector);
	int i;
	for(i = 0; i < attribute_vector.size(); i++){
		string index_name = attribute_vector[i].index;
		if(index_name != ""){ // 有索引
			int keytype = getType(attribute_vector[i].type);
			int blockOffset = index_manager->searchIndex(index_file_name_get(index_name), value_vector[i], keytype);
			if(blockOffset != -1){ // 索引中存在了这个值
				cout << "Error: There exsits same value" << endl;
				return;
			}
	...
	if(condition_vector.size() > 0){
		for(int i = 0; i < condition_vector.size(); i++){
			int record_conflict = record_manager->searchRecord(table_name, recordSize, bn, attribute_vector, condition_vector[i]);
			if(record_conflict > 0){
				cout << "Error: must be unique" << endl;
				return;
			}
		}
	}
	...
	int offset = record_manager->insertRecord(table_name, &record, recordSize);
	if(offset >= 0){
		catalog_manager->insertRecord(table_name, value_vector.size());
		cout << "Insert successfully" << endl;
	}
	else{
		cout << "insert fail" << endl;
...
{	
		...
		for(int i = 0; i < condition_vector.size(); i++){
			for(int j = 0; j < attribute_vector.size(); j++){
				if(attribute_vector[j].name == condition_vector[i].Attribute_Name && attribute_vector[j].index != ""){
					blockOffset = index_manager->searchIndex(index_file_name_get(attribute_vector[j].index), condition_vector[i].value, attribute_vector[j].type);
				}
			}
		}	
	}
	...
	if(blockOffset == -1){ // 没有索引
		...
		//DeleteInIndex(index_file_name_get(index_name), key, keytype);
	}
...
void API::update(string table_name, string attribute_name, string new_value, vector<Condition> &condition_vector)
{
	...
	if(condition_vector.size() != 0){
		for(int i = 0; i < condition_vector.size(); i++){
			...
			for(j = 0; j < attribute_vector.size(); j++){
				if(attribute_vector[j].name == condition_vector[i].Attribute_Name){
					if(condition_vector[i].operate == OPERATOR_EQUAL && attribute_vector[j].index != ""){
						index_name = index_file_name_get(attribute_vector[j].index);
	...
	// 没有索引
	if(offset == -1){
		string delete_value = record_manager->updateRecord(table_name, recordSize, attribute_vector, condition_vector);
	}
	else{// 有索引
		string delete_value = record_manager->updateRecord2(table_name, recordSize, attribute_vector, condition_vector, offset);
		//index_manager->DeleteInIndex(index_file_name_get(index_name), delete_value, getType(attribute_vector[update_position]));
		//index_manager->InsertIntoIndex(index_file_name_get(index_name), new_value, getType(attribute_vector[update_position]));
	}
