...
void BufferManager::updateBlock(string fileName, char *UpdateContent, int offsetInFile) {
	...
	for (int i = 0; i < 20; i++)
	{
		cout << bufferPool[offsetInBuffer].content[i];
	}
	setDirty(offsetInBuffer);

	writtenToDisk(offsetInBuffer);
}
...
int BufferManager::createNewBlock(string fileName) {
	int blockNum = countBlock(fileName);
	return blockNum - 1;//因为是offset，数组下标，所以少1
}

BufferManager bm;

//int main() {
//	int t = bm.createNewBlock("1.txt");
//	//cout<<t<<endl;
//	//char str[blockSize];
//	//memset(str,'0',blockSize-1);
//	char str[10] = { 'I',' ','a','m',' ','h','a','p','p','y' };
//	bm.updateBlock("1.txt", str, t);
//	bm.writtenToDisk(0);
//	cout << bm.getBlockByOffset("1.txt", t);
//	/*int s=bm.createNewBlock("1.txt");
//	cout<<s<<endl;
//	char dd[10]={ 'I',' ','a','m',' ','s','a','d','p','y'};
//	bm.updateBlock("1.txt",dd,s);
//	bm.writtenToDisk(1);
//	cout<<bm.getBlockByOffset("1.txt",s);
//	cout<<"welcome!"<<endl;*/
//	system("pause");
//}
