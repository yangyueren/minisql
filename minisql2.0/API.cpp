...
void API::record_insert(string table_name, vector<string> &value_vector)
{
	...
	// 从catalog获取record
	catalog_manager->getRecordString(table_name, &value_vector, record);
	...
	if(condition_vector.size() > 0){
		for(int i = 0; i < condition_vector.size(); i++){
			int record_conflict = record_manager->searchRecord(table_name, recordSize, bn, attribute_vector, condition_vector);
		...
		for(int i = 0; i < condition_vector.size(); i++){
			for(int j = 0; j < attribute_vector.size(); j++){
				if(attribute_vector[j].name == condition_vector[i].Attribute_Name && attribute_vector[j].index != ""){
					blockOffset = index_manager->SearchInIndex(index_file_name_get(attribute_vector[j].index), condition_vector[i].value, attribute_vector[j].type);
				}
			}
		}	
	}
