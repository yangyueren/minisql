...
public:
	...
	OffsetType SearchInIndex(string IndexName, string KeyValue, int KeyType);           //-1代表没有
	bool InsertIntoIndex(string IndexName, string KeyValue, int KeyType, OffsetType Offset);// string IndexName, string KeyValue, int KeyType, OffsetType Offset
	bool DeleteInIndex(string IndexName, string KeyValue, int KeyType);
	bool DeleteIndex(string IndexName);//在传入删除的索引文件名，在内存中删除索引头
private:
